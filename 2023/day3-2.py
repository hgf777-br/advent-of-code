...
data = """............677..........................................................................227.....730..35.......318...........92...166.......
...
def gear_ratio_value(data, line, line_number):
    sum_gear_ratios = 0
    for i in range(len(line)):
        part = line[i]
        if part[0] == '*':
            part_part_numbers = []
            if i > 0 and line[i - 1][0].isnumeric() and part[1] == line[i - 1][2]:
                part_part_numbers.append(int(line[i - 1][0]))
            if i < len(line) - 1 and line[i + 1][0].isnumeric() and line[i + 1][1] == part[2]:
                part_part_numbers.append(int(line[i + 1][0]))
            if line_number > 0:
                for part_number_up in data[line_number - 1]:
                    if part_number_up[1] > part[2]:
                        break
                    if part_number_up[0].isnumeric() and (
                        part_number_up[2] == part[1]
                        or part[1] in range(part_number_up[1], part_number_up[2])
                        or part_number_up[1] == part[2]
                    ):
                        part_part_numbers.append(int(part_number_up[0]))
            if line_number < len(data) - 1:
                for part_number_down in data[line_number + 1]:
                    if part_number_down[1] > part[2]:
                        break
                    if part_number_down[0].isnumeric() and (
                        part_number_down[2] == part[1]
                        or part[1] in range(part_number_down[1], part_number_down[2])
                        or part_number_down[1] == part[2]
                    ):
                        part_part_numbers.append(int(part_number_down[0]))
            if len(part_part_numbers) == 2:
                sum_gear_ratios += part_part_numbers[0] * part_part_numbers[1]

    print(line_number, sum_gear_ratios)

    return sum_gear_ratios


data = data.split('\n')
data = [re.finditer(r'(\d+|[^\d.])', line) for line in data]
data = [[(item.group(), item.start(), item.end()) for item in line] for line in data]
print(len(data))

total = 0
for line_number, line in enumerate(data):
    total += gear_ratio_value(data, line, line_number)
