...
data = """F-F-JJF7.F7.F-LJ7-|77.L|.FJ7--7LFL|-|F-.|--7.F7-F-.F|-FL7.FF7.F77-FLLJFLF--.FL--FF--7..FJ-L-LL-J..F-J77FF|-FF77FL|-|7J-7--7J-L77.FL.FL|-F7F-
...
data_test_5 = """FF7FSF7F7F7F7F7F---7
L|LJ||||||||||||F--J
FL-7LJLJ||||||LJL-77
F--JF--7||LJLJ7F7FJ-
L---JF-JLJ.||-FJLJJ7
...
def start_allowed_directions(data, start_position):
    max_y, max_x = data.shape
    allowed_directions = []
    for k, v in DIRECTIONS.items():
        new_position = start_position + v
        if 0 <= new_position[0] < max_y and 0 <= new_position[1] < max_x:
            match k:
                case 'n':
                    if data[new_position[0], new_position[1]] in ['|', 'F', '7']:
                        allowed_directions.append('n')
                case 'e':
                    if data[new_position[0], new_position[1]] in ['-', 'J', '7']:
                        allowed_directions.append('e')
                case 's':
                    if data[new_position[0], new_position[1]] in ['|', 'J', 'L']:
                        allowed_directions.append('s')
                case 'w':
                    if data[new_position[0], new_position[1]] in ['-', 'L', 'F']:
...
start_position = np.argwhere(data == 'S')[0]
start_directions = start_allowed_directions(data, start_position)
max_y, max_x = data.shape
...
position = start_position + DIRECTIONS[direction]
walk_count = 0
while not all(position == start_position):
    ...
    if 0 > position[0] >= max_y or 0 > position[1] >= max_x:
        break

path = [(x, y) for x, y in path]

if start_directions == ['e', 's']:
    data[start_position[0], start_position[1]] = 'F'
elif start_directions == ['s', 'w']:
    data[start_position[0], start_position[1]] = '7'
elif start_directions == ['n', 'w']:
    data[start_position[0], start_position[1]] = 'J'
elif start_directions == ['n', 'e']:
    data[start_position[0], start_position[1]] = 'L'
elif start_directions == ['n', 's']:
    data[start_position[0], start_position[1]] = '|'
elif start_directions == ['e', 'w']:
    data[start_position[0], start_position[1]] = '-'
