...
digits_regex = re.compile(r'\d|'+r"|".join(number_names))
digits_regex_reverse = re.compile(r'\d|'+r"|".join(n[::-1] for n in number_names))
answer = 0
for line in data:
    start_digit = digits_regex.search(line)[0]  # type: ignore
    end_digit = digits_regex_reverse.search(line[::-1])[0][::-1]  # type: ignore
    number = f'{number_names.get(start_digit, start_digit)}{number_names.get(end_digit, end_digit)}'
    print(number)
    answer += int(number)
