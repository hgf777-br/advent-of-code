...
data = """Game 1: 3 green, 1 blue, 3 red; 3 blue, 1 green, 3 red; 2 red, 12 green, 7 blue; 1 red, 4 blue, 5 green; 7 green, 2 blue, 2 red
...
powers = []
for game, line in enumerate(data, 1):
    cube_colors_qtd = {}
    line = line.split(':')[1]
    grabs = line.split(';')
    for grab in grabs:
        ...
        for cube in cubes:
            ...
            if color not in cube_colors_qtd:
                cube_colors_qtd[color] = int(qtd)
            else:
                if int(qtd) > cube_colors_qtd[color]:
                    cube_colors_qtd[color] = int(qtd)
    power = math.prod(cube_colors_qtd.values())
    powers.append(power)
