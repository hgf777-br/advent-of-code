...
lights = np.zeros((1000,1000), np.int8)

data = data.split('\n')

for data in data:
    data = data.split()
    adj = 1 if data[0] == 'turn' else 0
    pxs, pys = (int(p) for p in data[adj + 1].split(','))
    pxe, pye = (int(p) for p in data[adj + 3].split(','))

    if data[0] == 'turn':
        if data[1] == 'on':
            lights_add = np.ones((pxe+1-pxs, pye+1-pys), np.int8)
            lights[pxs:pxe+1, pys:pye+1] += lights_add
        else:
            partial_lights = lights[pxs:pxe+1, pys:pye+1]
            lights_sub = np.ones((pxe+1-pxs, pye+1-pys), np.int8)
            partial_lights -= lights_sub
            partial_lights = np.where(partial_lights>=0,partial_lights,partial_lights*0)
            lights[pxs:pxe+1, pys:pye+1] = partial_lights
    else:
        lights_add = np.ones((pxe+1-pxs, pye+1-pys), np.int8)
        lights[pxs:pxe+1, pys:pye+1] += lights_add
        lights[pxs:pxe+1, pys:pye+1] += lights_add
